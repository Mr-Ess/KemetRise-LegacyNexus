...
const NAV = [
  { group: "main", ar: "الرئيسية", en: "Main", items: [
    { icon: LayoutDashboard, ar: "لوحة التحكم",  en: "Dashboard",  path: "/staff" },
    { icon: Star,            ar: "ملفي الشخصي", en: "My Profile", path: "/staff/profile" },
  ]},
  { group: "work", ar: "العمل", en: "Work", items: [
    { icon: CheckSquare,   ar: "مهامي",          en: "My Tasks",   path: "/staff/tasks" },
    { icon: Calendar,      ar: "جدولي",           en: "Schedule",   path: "/staff/schedule" },
    { icon: Clock,         ar: "الحضور",          en: "Attendance", path: "/staff/attendance" },
  ]},
  { group: "communication", ar: "التواصل", en: "Communication", items: [
    { icon: MessageSquare, ar: "الرسائل",         en: "Messages",   path: "/staff/messages" },
    { icon: Bell,          ar: "الإشعارات",       en: "Notifications", path: "/staff/notifications" },
  ]},
];
...
export default function StaffLayout({ children }: { children: ReactNode }) {
  ...
  const [open, setOpen] = useState<Record<string, boolean>>({ main: true, work: true, communication: true });
    ...
    <div className={cn("flex h-screen bg-background overflow-hidden", R && "rtl")}>
      ...
      <aside className={cn("h-screen bg-sidebar border-border flex flex-col shrink-0 transition-all duration-300 border-r", collapsed ? "w-14" : "w-56")}>
        ...
        <div className="p-3 border-b border-border flex items-center justify-between shrink-0">
          {!collapsed && (
            <div className="flex items-center gap-2">
              ...
              <span className="font-display text-xs font-bold text-violet-400 tracking-widest">
                {R ? "موظف" : "STAFF"}
              </span>
            </div>
          )}
        ...
        {!collapsed && (
          <div className="px-3 py-2 border-b border-border/50 flex items-center gap-2">
            <div className="w-6 h-6 rounded-full bg-violet-500/20 border border-violet-500/40 flex items-center justify-center text-[10px] font-bold text-violet-400 shrink-0">
              {profile?.full_name?.charAt(0)?.toUpperCase() || "S"}
            </div>
            <div className="flex-1 min-w-0">
              <p className="text-xs font-semibold truncate">{profile?.full_name}</p>
              <Badge className="text-[8px] px-1 py-0 bg-violet-500/20 text-violet-400 border-violet-500/40">
                {R ? "موظف" : "STAFF"}
              </Badge>
            </div>
          </div>
      ...
      <div className="flex-1 flex flex-col min-w-0 overflow-hidden">
        <header className="h-12 border-b border-border bg-background/80 backdrop-blur-md flex items-center justify-between px-4 shrink-0">
          <span className="text-xs text-muted-foreground">{R ? "بوابة الموظفين" : "Staff Portal"}</span>
          <div className="flex items-center gap-2">
            <button
              onClick={() => navigate("/")}
