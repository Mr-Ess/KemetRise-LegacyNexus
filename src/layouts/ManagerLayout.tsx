...
import {
  ...
  LogOut, Menu, Bell, Languages, ChevronDown, FileText,
  Calendar, Globe, TrendingUp, MessageSquare, CheckSquare,
} from "lucide-react";
...
const NAV = [
  { group: "main", ar: "الرئيسية", en: "Main", items: [
    { icon: LayoutDashboard, ar: "لوحة التحكم",   en: "Dashboard",   path: "/manager" },
    { icon: BarChart3,       ar: "التحليلات",     en: "Analytics",   path: "/manager/analytics" },
  ]},
  { group: "team", ar: "الفريق", en: "Team", items: [
    { icon: Users,        ar: "إدارة الفريق",   en: "Team",         path: "/manager/team" },
    { icon: Calendar,     ar: "الجداول",         en: "Schedules",    path: "/manager/schedules" },
    { icon: CheckSquare,  ar: "المهام",           en: "Tasks",        path: "/manager/tasks" },
  ]},
  { group: "reports", ar: "التقارير", en: "Reports", items: [
    { icon: TrendingUp,   ar: "الأداء",          en: "Performance",  path: "/manager/performance" },
    { icon: FileText,     ar: "التقارير",        en: "Reports",      path: "/manager/reports" },
  ]},
  { group: "communication", ar: "التواصل", en: "Communication", items: [
    { icon: MessageSquare, ar: "الرسائل",        en: "Messages",     path: "/manager/messages" },
    { icon: ClipboardList, ar: "الإشعارات",      en: "Announcements",path: "/manager/announcements" },
  ]},
];
...
export default function ManagerLayout({ children }: { children: ReactNode }) {
  ...
  const [open, setOpen] = useState<Record<string, boolean>>({ main: true, team: true, reports: true, communication: true });
  const R = i18n.language === "ar";

  const active = (p: string) => location.pathname === p || location.pathname.startsWith(p + "/");
  const signout = async () => { await signOut(); navigate("/auth"); toast.success(R ? "تم تسجيل الخروج" : "Signed out"); };
    ...
    <div className={cn("flex h-screen bg-background overflow-hidden", R && "rtl")}>
      ...
      <aside className={cn("h-screen bg-sidebar border-border flex flex-col shrink-0 transition-all duration-300 border-r", collapsed ? "w-14" : "w-56")}>
        ...
        <div className="p-3 border-b border-border flex items-center justify-between shrink-0">
          {!collapsed && (
            <div className="flex items-center gap-2">
              ...
              <span className="font-display text-xs font-bold text-emerald-400 tracking-widest">
                {R ? "مدير" : "MANAGER"}
              </span>
            </div>
          )}
        ...
        {!collapsed && (
          <div className="px-3 py-2 border-b border-border/50 flex items-center gap-2">
            <div className="w-6 h-6 rounded-full bg-emerald-500/20 border border-emerald-500/40 flex items-center justify-center text-[10px] font-bold text-emerald-400 shrink-0">
              {profile?.full_name?.charAt(0)?.toUpperCase() || "M"}
            </div>
            <div className="flex-1 min-w-0">
              <p className="text-xs font-semibold truncate">{profile?.full_name}</p>
              <Badge className="text-[8px] px-1 py-0 bg-emerald-500/20 text-emerald-400 border-emerald-500/40">
                {R ? "مدير" : "MANAGER"}
              </Badge>
            </div>
          </div>
          ...
          {NAV.map(({ group, ar, en, items }) => (
            <div key={group}>
              {!collapsed && (
                ...
                >
                  {R ? ar : en}
                  <ChevronDown className={cn("w-3 h-3 transition-transform", !open[group] && "-rotate-90")} />
                </button>
              )}
        ...
        <div className="border-t border-border py-2 px-1.5 space-y-0.5 shrink-0">
          ...
          >
            ...
            {!collapsed && <span>{R ? "الإعدادات" : "Settings"}</span>}
          </button>
          <button
            onClick={signout}
          ...
          >
            ...
            {!collapsed && <span>{R ? "خروج" : "Sign Out"}</span>}
          </button>
        </div>
      </aside>
      ...
      <div className="flex-1 flex flex-col min-w-0 overflow-hidden">
        <header className="h-12 border-b border-border bg-background/80 backdrop-blur-md flex items-center justify-between px-4 shrink-0">
          <span className="text-xs text-muted-foreground">{R ? "بوابة المديرين" : "Manager Portal"}</span>
          <div className="flex items-center gap-2">
            <button
              onClick={() => navigate("/")}
            ...
            <button
              onClick={() => i18n.changeLanguage(R ? "en" : "ar")}
