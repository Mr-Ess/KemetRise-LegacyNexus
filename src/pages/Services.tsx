...
const Services = () => {
  ...
  const openEdit = (s: Service) => { const { id, ...rest } = s; setForm(rest); setContacts((s as any).contacts || []); setExtraAttachments((s as any).extraAttachments || []); setSocialAccounts((s as any).socialAccounts || []); setTeamDetails((s as any).teamDetails || []); setEditId(s.id); setShowForm(true); };
