...
const Branches = () => {
  ...
  const resetForm = () => { setForm(empty()); setContacts([]); setExtraAttachments([]); setSocialAccounts([]); setTeamDetails([]); setEditId(null); };
  const openEdit = (b: Branch) => { const { id, ...rest } = b; setForm(rest); setContacts((b as any).contacts || []); setExtraAttachments((b as any).extraAttachments || []); setSocialAccounts((b as any).socialAccounts || []); setTeamDetails((b as any).teamDetails || []); setEditId(b.id); setShowForm(true); };
