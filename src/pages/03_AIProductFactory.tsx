// src/pages/admin/AIProductFactory.tsx
//
// Drop this file in as-is. Then:
//   1. Add the lazy import + route in src/App.tsx           (see 04_App_and_AdminLayout_patch.md)
//   2. Add the nav item in src/layouts/AdminLayout.tsx        (see 04_App_and_AdminLayout_patch.md)
...
import {
  ...
  Package, Users, Zap, ExternalLink, ShieldCheck,
} from "lucide-react";
import { toast } from "sonner";
import { format } from "date-fns";
import { cn } from "@/lib/utils";

const FN_APPROVE_URL = "https://YOUR_PROJECT_REF.functions.supabase.co/ai-factory-approve";
...
export default function AIProductFactory() {
  ...
  useEffect(() => {
    load();
    // Live updates: refresh whenever approval queue or products change
    const ch = supabase
      .channel("aidpf-live")
  ...
  return (
    <AdminLayout>
      <div className={cn("p-6 space-y-6", R && "rtl")}>
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            <Sparkles className="w-5 h-5 text-primary" />
            <h1 className="text-xl font-bold">{R ? "مصنع المنتجات الرقمية الذكي" : "Autonomous AI Digital Product Factory"}</h1>
          </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          <Card><CardContent className="p-4 flex items-center justify-between">
            <div><p className="text-xs text-muted-foreground">{R ? "منشورة" : "Published"}</p><p className="text-2xl font-bold">{stats.published}</p></div>
            <CheckCircle2 className="w-6 h-6 text-emerald-400" />
          </CardContent></Card>
          <Card><CardContent className="p-4 flex items-center justify-between">
            <div><p className="text-xs text-muted-foreground">{R ? "قيد المراجعة" : "In Review"}</p><p className="text-2xl font-bold">{stats.inReview}</p></div>
            <Clock className="w-6 h-6 text-amber-400" />
          </CardContent></Card>
          <Card><CardContent className="p-4 flex items-center justify-between">
            <div><p className="text-xs text-muted-foreground">{R ? "مرفوضة" : "Rejected"}</p><p className="text-2xl font-bold">{stats.rejected}</p></div>
            <XCircle className="w-6 h-6 text-red-400" />
          </CardContent></Card>
          <Card><CardContent className="p-4 flex items-center justify-between">
            <div><p className="text-xs text-muted-foreground">{R ? "تصعيد تلقائي" : "Auto-Fallback"}</p><p className="text-2xl font-bold">{stats.autoFallback}</p></div>
        ...
        {/* Approval Queue */}
        <Card>
          <CardHeader><CardTitle className="text-sm flex items-center gap-2"><ShieldCheck className="w-4 h-4" />{R ? "بوابة الموافقات متعددة المستويات" : "Multi-Tier Approval Gateway"}</CardTitle></CardHeader>
          <CardContent className="space-y-2">
            {queue.length === 0 && <p className="text-xs text-muted-foreground">{R ? "لا توجد طلبات" : "No approval requests yet."}</p>}
            {queue.map((row) => {
        ...
        {/* Products */}
        <Card>
          <CardHeader><CardTitle className="text-sm flex items-center gap-2"><Package className="w-4 h-4" />{R ? "المنتجات المولّدة" : "Generated Products"}</CardTitle></CardHeader>
          <CardContent className="space-y-2">
            {products.map((p) => (
        ...
        {/* Live Pipeline Log */}
        <Card>
          <CardHeader><CardTitle className="text-sm flex items-center gap-2"><AlertTriangle className="w-4 h-4" />{R ? "سجل التنفيذ" : "Pipeline Logs"}</CardTitle></CardHeader>
