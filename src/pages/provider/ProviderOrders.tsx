...
export default function ProviderOrders() {
  ...
  const load = useCallback(async () => {
    ...
    let q: any = orderIds.length
      ? db.from("mp_orders").select("*").in("id", orderIds).order("created_at", { ascending: false }).range(page * PAGE_SIZE, (page + 1) * PAGE_SIZE - 1)
      : null;
    if (!q) { setOrders([]); setLoading(false); return; }
      ...
      <div className="p-6 space-y-6">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-sm text-muted-foreground">{R ? "إدارة طلبات نشاطك" : "Manage your business orders"}</p>
          </div>
        ...
        <div className="flex flex-wrap gap-2">
          {[{ v: "all", l: R ? "الكل" : "All" }, ...Object.entries(STATUS_CONFIG).map(([k, v]) => ({ v: k, l: R ? v.labelAr : v.label }))].map(opt => (
            <Button key={opt.v} size="sm" variant={filter === opt.v ? "default" : "outline"}
              className={cn(filter === opt.v && "bg-blue-600 hover:bg-blue-700")}
        ...
        {loading ? (
          <div className="space-y-3">{[1,2,3,4,5].map(i => <div key={i} className="h-14 bg-muted/30 rounded-xl animate-pulse" />)}</div>
        ) : orders.length === 0 ? (
                  ...
                  <div key={order.id} className="grid grid-cols-5 px-4 py-3 items-center text-sm hover:bg-muted/10 transition-colors">
                    ...
                    <div className="flex justify-end">
                      <Select value={order.status} onValueChange={v => updateStatus(order.id, v)}>
                        <SelectTrigger className={cn("h-7 text-xs w-32 border", sc.color)}><SelectValue /></SelectTrigger>
                        <SelectContent>{Object.entries(STATUS_CONFIG).map(([k, v]) => <SelectItem key={k} value={k}>{R ? v.labelAr : v.label}</SelectItem>)}</SelectContent>
                      </Select>
                    </div>
                  </div>
                );
