...
import { BarChart3, DollarSign, ShoppingBag, Package, TrendingUp, RefreshCcw, Star, Users } from "lucide-react";
...
export default function ProviderAnalytics() {
  ...
  const [stats, setStats] = useState({ listings: 0, activeListings: 0, totalRevenue: 0, totalOrders: 0, completedOrders: 0, avgOrder: 0, convRate: 0 });
  ...
  const load = useCallback(async () => {
    ...
    // Get listing counts
    const [{ count: listings }, { count: activeListings }] = await Promise.all([
    ...
    // Get orders via join: mp_listings → mp_order_items → mp_orders
    ...
    const { data: ord } = orderIds.length
      ? await db.from("mp_orders").select("total_cents,status,created_at").in("id", orderIds).gte("created_at", since)
      : { data: [] };
    ...
    setOrders(list);
    setStats({ listings: listings ?? 0, activeListings: activeListings ?? 0, totalRevenue: revenue, totalOrders: list.length, completedOrders: completed.length, avgOrder: completed.length > 0 ? revenue / completed.length : 0, convRate: list.length > 0 ? (completed.length / list.length) * 100 : 0 });
    setLoading(false);
  }, [profile, period]);
  ...
  const kpis = [
    { label: R ? "الإدراجات" : "Listings",       value: stats.listings,                              icon: Package,    color: "text-blue-400"   },
    { label: R ? "نشطة" : "Active",               value: stats.activeListings,                        icon: TrendingUp, color: "text-green-400"  },
    { label: R ? "الطلبات" : "Total Orders",       value: stats.totalOrders,                           icon: ShoppingBag,color: "text-indigo-400" },
    { label: R ? "مكتملة" : "Completed",           value: stats.completedOrders,                       icon: TrendingUp, color: "text-emerald-400"},
    { label: R ? "الإيرادات" : "Revenue",          value: `$${(stats.totalRevenue / 100).toFixed(2)}`, icon: DollarSign, color: "text-orange-400" },
    { label: R ? "معدل الإتمام" : "Completion %",  value: `${stats.convRate.toFixed(1)}%`,             icon: BarChart3,  color: "text-purple-400" },
  ];
        ...
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-2xl font-display font-black flex items-center gap-2">
              ...
              {R ? "الإحصائيات" : "Analytics"}
            </h1>
            <p className="text-sm text-muted-foreground">{R ? "أداء نشاطك التجاري" : "Your business performance"}</p>
          </div>
