...
function DeveloperHubTab() {
  ...
  // White Label
  const [wl, setWl] = React.useState<any>({
    ...
    <div className="space-y-4">
      ...
      <div className="flex items-center justify-between flex-wrap gap-2">
        <div className="flex items-center gap-3 p-3 bg-primary/5 border border-primary/20 rounded-lg flex-1">
          ...
          <div>
            <h3 className="font-display text-sm font-bold">Developer Hub</h3>
            <p className="text-[11px] text-muted-foreground font-mono">{DEV_BASE}</p>
          </div>
        </div>
      ...
      <Tabs defaultValue="docs">
        ...
        <TabsContent value="docs" className="space-y-4 mt-4">
          <Tabs defaultValue="quickstart">
            ...
            <TabsContent value="quickstart" className="space-y-3 mt-3">
              <Card className="p-4 space-y-2"><h3 className="font-bold text-sm">1. Create an API Key</h3><p className="text-xs text-muted-foreground">Go to <strong>API Keys</strong> tab → New Key. The key is shown once — save it immediately.</p></Card>
              ...
              <Card className="p-4 space-y-2"><h3 className="font-bold text-sm">3. Fetch Data (JavaScript)</h3><DevCodeBlock>{`const res = await fetch("${DEV_BASE}/invoices?select=*&limit=50", {\n  headers: {\n    apikey: "YOUR_API_KEY",\n    Authorization: "Bearer YOUR_API_KEY"\n  }\n});\nconst invoices = await res.json();`}</DevCodeBlock></Card>
              <Card className="p-4 space-y-2"><h3 className="font-bold text-sm">4. Filter &amp; Sort</h3><DevCodeBlock>{`# Filter by status\nGET ${DEV_BASE}/tasks?status=eq.in-progress\n\n# Sort descending\nGET ${DEV_BASE}/audit_logs?order=created_at.desc\n\n# Pagination\nGET ${DEV_BASE}/customers?limit=20&offset=40`}</DevCodeBlock></Card>
              <Card className="p-4 space-y-2"><h3 className="font-bold text-sm">5. Python example</h3><DevCodeBlock>{`import requests\n\nresp = requests.get(\n    "${DEV_BASE}/brands",\n    headers={\n        "apikey": "YOUR_API_KEY",\n        "Authorization": "Bearer YOUR_API_KEY"\n    }\n)\nprint(resp.json())`}</DevCodeBlock></Card>
            </TabsContent>
        ...
        <TabsContent value="keys" className="mt-4 space-y-3">
          ...
          {newKey && (
            <Card className="p-4 border border-emerald-500 bg-emerald-500/5">
              ...
              <DevCodeBlock>{newKey}</DevCodeBlock>
          ...
          <Card className="overflow-x-auto">
            <table className="w-full text-sm">
              ...
              <tbody>
                {apiKeys.length===0
                  ?<tr><td colSpan={4} className="p-8 text-center text-muted-foreground">No API keys yet</td></tr>
                  :apiKeys.map((k:any)=>(
        ...
        <TabsContent value="webhooks" className="mt-4 space-y-3">
          ...
          <Card>
            {hooks.length===0
              ?<p className="p-8 text-center text-muted-foreground text-sm">No webhooks configured</p>
        ...
        <TabsContent value="deliveries" className="mt-4">
          <Card className="overflow-x-auto">
            <table className="w-full text-sm">
              ...
              <tbody>
                {deliveries.length===0
                  ?<tr><td colSpan={5} className="p-8 text-center text-muted-foreground">No delivery attempts yet</td></tr>
            ...
            :<div className="max-w-2xl space-y-4">
              <div className="flex items-center gap-3 mb-4">
                ...
                <div>
                  ...
                  <p className="text-xs text-muted-foreground">Customize the platform with your own brand identity.</p>
                </div>
              </div>
              {/* Preview */}
              ...
              <Card className="p-5 space-y-4">
                ...
                <div className="flex items-center justify-between p-3 border border-border rounded-md">
                  <div><p className="text-sm font-medium">Hide "Powered by KemetRise"</p><p className="text-xs text-muted-foreground">Premium plan required</p></div>
        ...
        <TabsContent value="extension" className="mt-4">
          <div className="max-w-2xl space-y-4">
            <div className="flex items-center gap-3 mb-4">
              ...
              <div><h2 className="font-display text-sm text-primary">Browser Extension</h2><p className="text-xs text-muted-foreground">Quick-access KemetRise from any tab — Chrome, Edge, Brave, Arc, Opera.</p></div>
            </div>
            <Card className="p-6 text-center bg-gradient-to-br from-primary/5 to-transparent">
              ...
              }}><Download className="w-4 h-4 mr-2"/>Download Extension</Button>
            </Card>
            <Card className="p-5">
              <h3 className="font-display text-sm text-primary mb-4">Installation Steps</h3>
              <ol className="space-y-3">
                {["Unzip the downloaded file.","Open \"chrome://extensions\" in Chrome (or Edge, Brave, Arc).","Enable Developer mode (toggle in top-right corner).","Click \"Load unpacked\" and select the unzipped folder.","The KemetRise icon will appear in your toolbar — click to open."].map((s,i)=>(
            ...
            <Card className="p-5">
              ...
              <div className="grid grid-cols-2 gap-2">
                {["One-click login","Quick task creation","Notification badge","Clipboard shortcuts","Offline mode support","Dark / light theme sync"].map(f=>(
                  <div key={f} className="flex items-center gap-2 text-sm"><Check className="w-3.5 h-3.5 text-emerald-400 shrink-0"/>{f}</div>
                ))}
              </div>
            </Card>
          </div>
              ...
              <div className="flex flex-wrap gap-2 mt-2">
                {events.map(e=>(
                  <button key={e} type="button"
                    onClick={()=>setHookForm(p=>({...p,events:p.events.includes(e)?p.events.filter(x=>x!==e):[...p.events,e]}))}
