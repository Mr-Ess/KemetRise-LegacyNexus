...
const NAV = [
  { group: "main", ar: "الرئيسية", en: "Main", items: [
    { icon: LayoutDashboard, ar: "لوحتي",       en: "My Dashboard",   path: "/agent" },
    { icon: TrendingUp,      ar: "أدائي",        en: "My Performance", path: "/agent/performance" },
  ]},
  { group: "clients", ar: "العملاء", en: "Clients", items: [
    { icon: Users,       ar: "عملائي",        en: "My Clients",     path: "/agent/clients" },
    { icon: ClipboardList,ar: "تأهيل العملاء", en: "Onboarding",    path: "/agent/onboarding" },
    { icon: Star,        ar: "ترقية العملاء", en: "Conversions",    path: "/agent/conversions" },
  ]},
  { group: "ops", ar: "العمليات", en: "Operations", items: [
    { icon: DollarSign, ar: "عمولاتي",     en: "Commissions",    path: "/agent/commissions" },
    { icon: MapPin,     ar: "مناطقي",      en: "My Regions",     path: "/agent/regions" },
    { icon: Ticket,     ar: "التذاكر",     en: "Tickets",        path: "/agent/tickets" },
  ]},
];
...
export default function AgentLayout({ children }: { children: ReactNode }) {
  ...
  const [open, setOpen] = useState<Record<string, boolean>>({ main: true, clients: true, ops: true });
    ...
    <div className={cn("flex h-screen bg-background overflow-hidden", R && "rtl")}>
      <aside className={cn("h-screen bg-sidebar border-border flex flex-col shrink-0 transition-all duration-300 border-r", collapsed ? "w-14" : "w-56")}>
        <div className="p-3 border-b border-border flex items-center justify-between shrink-0">
          {!collapsed && <div className="flex items-center gap-2"><MapPin className="w-4 h-4 text-emerald-400" /><span className="font-display text-xs font-bold text-emerald-400 tracking-widest">{R ? "وكيل" : "AGENT"}</span></div>}
        ...
        {!collapsed && (
          <div className="px-3 py-2 border-b border-border/50 flex items-center gap-2">
            <div className="w-6 h-6 rounded-full bg-emerald-500/20 border border-emerald-500/40 flex items-center justify-center text-[10px] font-bold text-emerald-400 shrink-0">{profile?.full_name?.charAt(0)?.toUpperCase() || "A"}</div>
            <div className="flex-1 min-w-0">
              <p className="text-xs font-semibold truncate">{profile?.full_name}</p>
              <Badge className="text-[8px] px-1 py-0 bg-emerald-500/20 text-emerald-400 border-emerald-500/40">AGENT</Badge>
            </div>
          </div>
        )}
            ...
            <div key={group}>
              ...
              {(collapsed || open[group] !== false) && items.map(item => (
                <button key={item.path} onClick={() => navigate(item.path)}
                  className={cn("w-full flex items-center gap-2.5 px-2 py-2 rounded-md text-xs transition-all", active(item.path) ? "bg-emerald-500/15 text-emerald-400 font-semibold" : "text-muted-foreground hover:text-foreground hover:bg-secondary/50")}
        ...
        <div className="border-t border-border py-2 px-1.5 space-y-0.5 shrink-0">
          <button onClick={() => navigate("/agent/settings")} className="w-full flex items-center gap-2.5 px-2 py-2 rounded-md text-xs text-muted-foreground hover:text-foreground hover:bg-secondary/50 transition-all"><Settings className="w-3.5 h-3.5 shrink-0" />{!collapsed && <span>{R ? "الإعدادات" : "Settings"}</span>}</button>
