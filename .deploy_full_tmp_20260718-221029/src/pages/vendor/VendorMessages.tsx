...
export default function VendorMessages() {
  ...
  const load = useCallback(async () => {
    if (!user) return;
    setLoading(true);
    const { data } = await db.from("support_tickets").select("*").eq("user_id", user.id).eq("category", "vendor").order("created_at", { ascending: false });
    setTickets(data ?? []);
    setLoading(false);
  }, [user]);
  ...
  const createTicket = async () => {
    ...
    const { error } = await db.from("support_tickets").insert({ subject: newSubject, description: newBody, category: "vendor", status: "open", user_id: user!.id, priority: "medium" });
      ...
      <div className="p-6 space-y-6">
        <div className="flex items-center justify-between">
          ...
          <div className="flex gap-2">
            ...
            <Button size="sm" onClick={() => setModal("new")} className="gap-2 bg-orange-600 hover:bg-orange-700">
        ...
        {loading ? (
          ...
          <div className="text-center py-16">
            ...
            <Button size="sm" onClick={() => setModal("new")} className="mt-4 bg-orange-600 hover:bg-orange-700">{R ? "أرسل رسالة" : "Send a Message"}</Button>
          </div>
        ) : (
      ...
      <Dialog open={modal === "new"} onOpenChange={() => setModal(null)}>
        <DialogContent className="max-w-md">
          ...
          <div className="space-y-3">
            ...
            <Textarea placeholder={R ? "محتوى الرسالة..." : "Message body..."} value={newBody} onChange={e => setNewBody(e.target.value)} rows={4} />
          </div>
          <div className="flex justify-end gap-2 pt-2">
            <Button variant="outline" onClick={() => setModal(null)}>{R ? "إلغاء" : "Cancel"}</Button>
            <Button onClick={createTicket} className="bg-orange-600 hover:bg-orange-700 gap-2"><Send className="w-4 h-4" />{R ? "إرسال" : "Send"}</Button>
          </div>
        </DialogContent>
      </Dialog>
      ...
      <Dialog open={modal === "view"} onOpenChange={() => setModal(null)}>
        <DialogContent className="max-w-lg">
          ...
          <div className="space-y-2 max-h-60 overflow-y-auto">
            {replies.map(r => (
              <div key={r.id} className={cn("px-3 py-2 rounded-lg text-sm max-w-[85%]", r.is_staff ? "bg-muted/40 text-muted-foreground" : "bg-orange-500/10 text-orange-300 ml-auto text-right")}>
                <p>{r.message}</p>
          ...
          {selected?.status !== "closed" && (
            <div className="flex gap-2 pt-2 border-t border-border/50">
              ...
              <Button size="sm" onClick={sendReply} className="bg-orange-600 hover:bg-orange-700 shrink-0 gap-1"><Send className="w-3 h-3" /></Button>
            </div>
          )}
        </DialogContent>
      </Dialog>
